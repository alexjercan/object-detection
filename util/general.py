...
def build_map_boxes(predictions, labels, train_idx, conf_thres, iou_thres, device):
    ...
    idxs = list(range(train_idx, train_idx + bs))
    train_idx = train_idx + bs
    
    predictions = torch.cat(
        [p.reshape(p.shape[0], -1, p.shape[4]) for p in predictions], 1)
    predictions = non_max_suppression(
        predictions, conf_thres=conf_thres, iou_thres=iou_thres, multi_label=True)
