...
from dataset.blender_dataset import BlenderDataset, segmentation2rgb
...
def print_info(images, bboxes, rbboxes, seg_masks, rseg_masks, predictions, classes):
    ...
    npsegs = np.transpose(np.array(segmentation2rgb(seg_masks)), (0, 2, 3, 1))
    nprsegs = np.transpose(
        np.array(segmentation2rgb(rseg_masks)), (0, 2, 3, 1))
