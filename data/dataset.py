...
def create_dataloader(img_dir, label_dir, image_size, batch_size, transform, used_layers=[L_RGB]):
    dataset = BDataset(img_dir, label_dir, image_size=image_size, transform=transform, used_layers=used_layers)
    loader = DataLoader(dataset=dataset, batch_size=batch_size, shuffle=True, collate_fn=BDataset.collate_fn)
    return dataset, loader


class BDataset(Dataset):
    def __init__(self, img_dir, label_dir, image_size=416, transform=None, used_layers=[L_RGB]):
